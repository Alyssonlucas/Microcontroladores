...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Questão 1 - Configuração do conversor AD
  * @author         : Alysson Lucas Pontes Cavalcante da Silva e Maria Victória Martins Neves
  * @date           : 06/05/2026
  * @details        : Aprendendo a realizar a conversão do conversor Analógico para digital.
  ******************************************************************************
  */
...
int main(void)
{
  ...
  MX_ADC2_Init();
  /* USER CODE BEGIN 2 */
  HAL_ADC_Start(&hadc2); // inicialização da captura do ADC2
  ...
  while (1)
  {
      ...
      if(HAL_ADC_PollForConversion(&hadc2, 0) == HAL_OK) // Verifica se a conversão foi realizada
      {
          valor_adc = HAL_ADC_GetValue(&hadc2); // Salva o valor na variável

          LedComportamento(valor_adc);

          HAL_ADC_Start(&hadc2); // Inicializa uma nova conversão
      }
  /* USER CODE END WHILE */
