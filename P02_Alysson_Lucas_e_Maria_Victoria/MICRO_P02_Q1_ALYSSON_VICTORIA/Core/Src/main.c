...
/**
  ...
  * @date           : 25/03/2026
  * @description    : Código contendo a inicialização do clock e configuração
  * dos pinos de entrada (botões) e saída (LEDs).
  ******************************************************************************
  */
...
void SystemClock_Config(void)
{
  ...
  */
  ...
  RCC_OscInitStruct.HSEPredivValue = RCC_HSE_PREDIV_DIV1; // Não divide a entrada (mantém 8MHz)
  RCC_OscInitStruct.HSIState = RCC_HSI_ON;

  // Ativa o PLL (Phase-Locked Loop) para multiplicar a frequência.
  // Pega os 8MHz do HSE e multiplica por 9 (PLL_MUL9), resultando na velocidade máxima: 72 MHz.
  ...
  */
  ...
  // Define o PLL (72 MHz) como a fonte de clock principal do sistema (SYSCLK)
  RCC_ClkInitStruct.SYSCLKSource = RCC_SYSCLKSOURCE_PLLCLK;

  // Barramento AHB (Processador e Memória): Não divide (Roda a 72 MHz)
  RCC_ClkInitStruct.AHBCLKDivider = RCC_SYSCLK_DIV1;
  // Barramento APB1 (Periféricos lentos): Divide por 2 (Roda a 36 MHz, que é o limite dele)
  RCC_ClkInitStruct.APB1CLKDivider = RCC_HCLK_DIV2;
  // Barramento APB2 (Periféricos rápidos): Não divide (Roda a 72 MHz)
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  // Garante que os LEDs externos (led1 a led4 na porta B) iniciem em nível ALTO (SET).
  HAL_GPIO_WritePin(GPIOB, led1_Pin|led2_Pin|led3_Pin|led4_Pin, GPIO_PIN_SET);

  /*Configure GPIO pin : UserLed_Pin */
  // 3. CONFIGURAÇÃO DO LED DA PLACA:
  // Modo Output Push-Pull (pode fornecer e drenar corrente), sem resistores de pull, baixa velocidade de chaveamento.
  ...
  // 4. CONFIGURAÇÃO DOS BOTÕES (ENTRADAS):
